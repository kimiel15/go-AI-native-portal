...
function RankedCard({ sub, rank }: { sub: RankedSubmission; rank: number }) {
        ...
        <div className="flex-1 min-w-0">
          ...
          {isUnscored ? (
            <p className="text-slate-400 text-xs flex items-center gap-1 mt-0.5">
              <AlertCircle className="w-3 h-3" /> No scores yet
            </p>
          ) : (
            <p className="text-slate-400 text-xs mt-0.5 flex items-center gap-1">
              <Users className="w-3 h-3" />
              {sub.judgeCount} judge{sub.judgeCount !== 1 ? 's' : ''}
            </p>
          )}
        </div>
          ...
          {isUnscored ? (
            ...
            <div>
              <span className={`text-2xl font-black ${
                sub.avgTotal >= 80 ? 'text-emerald-600' :
                sub.avgTotal >= 60 ? 'text-amber-500' :
                                     'text-red-500'
              }`}>{sub.avgTotal}</span>
                  ...
                  <div key={s.judgeId} className="bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className={`text-sm font-bold ${
                        s.totalScore >= 80 ? 'text-emerald-600' :
                        s.totalScore >= 60 ? 'text-amber-500' :
                                             'text-red-500'
                      }`}>{s.totalScore}%</span>
                    </div>
                    ...
                    {s.notes && (
                      <p className="text-slate-500 text-xs italic border-t border-gray-200 pt-2">&ldquo;{s.notes}&rdquo;</p>
                    )}
                  </div>
                ))}
              </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-10">
        <div className="mb-8">
          ...
          <p className="text-slate-500 text-sm">
            Ranked by average weighted score across all judges. Expand a row to see per-judge breakdowns.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {rankings.map((sub, i) => (
              <RankedCard key={sub.id} sub={sub} rank={i + 1} />
            ))}
          </div>
        )}
      </div>
